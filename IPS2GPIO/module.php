...
class IPS2GPIO_IO extends IPSModule
{
	...
	private function Get_PinUpdate()
	{
		...
		If (($this->ReadPropertyBoolean("I2C_Used") == true) AND (GetValueInteger($this->GetIDForIdent("HardwareRev"))) <= 3) {
			$PinUsed[] = 0; 
			$PinUsed[] = 1;
			$this->CommandClientSocket(pack("LLLL", 0, 0, "ALT0", 0), 16);
			$this->CommandClientSocket(pack("LLLL", 0, 1, "ALT0", 0), 16);
		}
		elseif (($this->ReadPropertyBoolean("I2C_Used") == true) AND (GetValueInteger($this->GetIDForIdent("HardwareRev"))) > 3) {
			$PinUsed[] = 2; 
			$PinUsed[] = 3;
			$this->CommandClientSocket(pack("LLLL", 0, 2, "ALT0", 0), 16);
			$this->CommandClientSocket(pack("LLLL", 0, 3, "ALT0", 0), 16);
		}
		elseif ($this->ReadPropertyBoolean("Serial_Used") == true)  {
			$PinUsed[] = 14; 
			$PinUsed[] = 15;
			$this->CommandClientSocket(pack("LLLL", 0, 14, "ALT0", 0), 16);
			$this->CommandClientSocket(pack("LLLL", 0, 15, "ALT0", 0), 16);
