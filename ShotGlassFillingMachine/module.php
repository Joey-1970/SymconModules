...
class ShotGlassFillingMachine extends IPSModule 
{
		...
		If ((IPS_GetKernelRunlevel() == KR_READY) AND ($this->HasActiveParent() == true)) {	
			...
			for ($i = 1; $i <= 5; $i++) {
				If (($this->ReadPropertyInteger("Pin_IRSensor_".$i) >= 0) AND ($this->ReadPropertyBoolean("Open") == true)) {
					$Result = $this->SendDataToParent(json_encode(Array("DataID"=> "{A0DAAF26-4A2D-4350-963E-CC02E74BD414}", "Function" => "set_usedpin", 
										  "Pin" => $this->ReadPropertyInteger("Pin_IRSensor_".$i), "PreviousPin" => $this->GetBuffer("PreviousPin_IRSensor_".$i), "InstanceID" => $this->InstanceID, "Modus" => 0, "Notify" => true, "GlitchFilter" => 200, "Resistance" => 2)));
	...
	public function SetHTMLDisplay()
	{
		...
		$EventTextColor = "#".substr("000000".dechex($this->ReadPropertyInteger("EventTextColor")),-6);
		...
		$HTMLText .= '<td style="width: 100%; height: 18px; text-align: center;border-style: hidden;" colspan="5"><h1>'.$StatusText.'</h1></td>';
		$HTMLText .= '</tr>';
		...
		$HTMLText .= '<tbody>';
		$HTMLText .= '<tr style="height: 18px; border-style: hidden;">';
		$HTMLText .= '<td style="width: 20%; height: 18px; text-align: center; vertical-align: middle; border-style: hidden;"><h3 onclick="window.xhrGet=function xhrGet(o) {var HTTP = new XMLHttpRequest();HTTP.open(\'GET\',o.url,true);HTTP.send();};window.xhrGet({ url: \'/hook/ShotGlassFillingMachine_'.$this->InstanceID.'?Action=AllShotOne\' })" >Alle Gläser mit '.$ShotName_1.' füllen</h3>"</td>';
		$HTMLText .= '<td style="width: 20%; height: 18px; text-align: center; vertical-align: middle; border-style: hidden;"><h3 onclick="window.xhrGet=function xhrGet(o) {var HTTP = new XMLHttpRequest();HTTP.open(\'GET\',o.url,true);HTTP.send();};window.xhrGet({ url: \'/hook/ShotGlassFillingMachine_'.$this->InstanceID.'?Action=AllShotTwo\' })" >Alle Gläser mit '.$ShotName_2.' füllen</h3>"</td>';
